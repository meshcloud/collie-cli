...
async function showUntagged(
  ...
  while (running) {
    ...
    if (selectedTag == "BACK") {
      ...
      while (runningInside) {
        const selectedTenant = await selectTenant(untaggedTenant, undefined, sortByCost);
...
async function selectTenant(
  ...
  for (const tenant of tenants) {
    ...
    if(sortByCost == true) {
      selectName = tenant.platformTenantName +' - Cost: '+ getTotal(tenant);
    } else {
      selectName = tenant.platformTenantName;
    }
    options.push({
